...
def _insert(data):
    ...
    # get uid via informix technique for obtaining the latest id
    # from a table
    ...
    if data["contact"]["previous_name"]:
        sql = """
            ...
            VALUES
                (%s,"M","%s")
        """ % (apptmp_no,data["contact"]["previous_name"])
    if settings.DEBUG:
        logger.debug("previous name sql = %s" % sql)
    connection.execute(sql)

    # Employment
    if data["employment"]["employer"]:
        sql = """
            INSERT INTO app_emptmp_rec (
                id, bus, pos
            )
            VALUES
                (%s,"%s","%s")
    ...
    if settings.DEBUG:
        logger.debug("employment sql = %s" % sql)
    connection.execute(sql)
    ...
    sql = """
        INSERT INTO app_admtmp_rec (
            ...
            vet_ben, add_date, parent_contr, enrstat, rank, emailaddr, major,
            prog, subprog, prg_of_study1, upd_uid, add_uid, upd_date,
            act_choice, stuint_wt, jics_candidate
        )
        VALUES (
            %s,"Y", "%s", "%s", "4", "%s", "%s", "0.00", "", "0",
            "%s", "%s", "%s", "%s", "%s", "0", "0", "%s", "", "0", "N"
        ) """ % (
            apptmp_no,plan_enr_sess,plan_enr_yr,
    ...
    # personal info
    # TODO:
    # birthplace_city, birthplace_st, birthplace_ctry
    sql = """
